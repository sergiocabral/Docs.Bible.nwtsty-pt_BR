...
async function addComment() {
  ...
  content.parentNode.insertBefore(remarkDiv, content.nextSibling);

  window.remark_config = {
    host: process.env.NEXT_PUBLIC_REMARK42_URL,
    site_id: process.env.NEXT_PUBLIC_REMARK42_SITE_ID,
    theme: document.documentElement.classList.contains('dark')
      ? 'dark'
      : 'light',
    no_footer: true,
    page_title: location.href.replace(/^\w+:\/\/|\/$/g, ''),
  };
...
async function setupDocumentUI() {
  ...
  baguetteBox.run('.imageblock,.image');
